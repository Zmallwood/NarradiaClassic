...
namespace Narradia {
   // clang-format off
   auto nMeshObtainer::ModelMeshIds(
         const aiScene *_rawModel) const -> Map<SharedPtr<String>, Vec<int>> {
      // clang-format on
   ...
   // clang-format off
   auto nMeshObtainer::NodeMeshes(
         const aiScene *_rawModel,
         Vec<int> _nodeMeshIDs) const -> Vec<aiMesh *> {
      // clang-format on
