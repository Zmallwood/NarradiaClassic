...
#include "Engine/Rendering/Rendering_Core.h"
#include "Math/Calc.h"
...
namespace Narradia {
    ...
    Point3F Camera::GetCameraPos() {
        ...
        Point3F result;
        {
            ...
            if (use_fixed_camera_distance_)
                ...
                result =
                    player_position_no_elevation.Translate(dx, dy + player_average_elevation, dz);
        }
        ...
        for (auto yy = -1; yy <= 1; yy++) {
            for (auto xx = -1; xx <= 1; xx++) {
                ...
                for (auto x = 0; x < wa->Width(); x += inc) {
                    rids_tiles.push_back(Vector<Render_ID>());

                    if (xx == 0 && yy == 0)
                        rids_tile_symbols.push_back(Vector<Render_ID>());
                    else if (xx == 0 && yy == -1)
                        rids_tile_symbols_n.push_back(Vector<Render_ID>());
                    else if (xx == 0 && yy == 1)
                        rids_tile_symbols_s.push_back(Vector<Render_ID>());
                    else if (xx == 1 && yy == 0)
                        rids_tile_symbols_e.push_back(Vector<Render_ID>());
                    else if (xx == -1 && yy == 0)
                        rids_tile_symbols_w.push_back(Vector<Render_ID>());
                    else if (xx == 1 && yy == -1)
                        rids_tile_symbols_ne.push_back(Vector<Render_ID>());
                    else if (xx == 1 && yy == 1)
                        rids_tile_symbols_se.push_back(Vector<Render_ID>());
                    else if (xx == 1 && yy == -1)
                        rids_tile_symbols_sw.push_back(Vector<Render_ID>());
                    else if (xx == -1 && yy == -1)
                        rids_tile_symbols_nw.push_back(Vector<Render_ID>());
                    ...
                    for (auto y = 0; y < wa->Height(); y += inc) {
                        ...
                        verts._00.pos = {
                            map_offset_x + x * t_sz, elevs._00, map_offset_y + y * t_sz};
                        verts._10.pos = {
                            map_offset_x + x * t_sz + t_sz_side, elevs._10,
                            map_offset_y + y * t_sz};
                        verts._11.pos = {
                            map_offset_x + x * t_sz + t_sz_side, elevs._11,
                            map_offset_y + y * t_sz + t_sz_side};
                        verts._01.pos = {
                            map_offset_x + x * t_sz, elevs._01,
                            map_offset_y + y * t_sz + t_sz_side};
            ...
            if (wa->IsInsideMap({x, y})) {
                ...
                if (tile) {
                    ...
                    if (!rid) {
                        ...
                        v2.pos = {
                            map_offs_x + x * t_sz + t_sz_side, elev11,
                            map_offs_y + y * t_sz + t_sz_side};
    ...
    inline void WorldAddV::DrawGround(Shared_Ptr<Tile> tile, Point coord) {
        ...
        if (ground == "GroundWater") {
            ...
            if (anim_index > 0)
                ground = "GroundWater_" + To_String(anim_index);
        }

        if (ground == "GroundGrass") {
            auto vary_index = (coord.x * coord.y) % 3;
            ground = "GroundGrass_" + To_String(vary_index);
        }
    ...
    void WorldAddV::DrawObjects(Shared_Ptr<Tile> tile, Point coord, int dloc_x, int dloc_y) {
        if (tile->object()) {
            auto curr_wa = World::get()->CurrWorldArea();
            ...
            auto model_scaling =
                0.6f * Objects_Conf::get()->GetModelScaling(tile->object()->type());
            DrawModel(
    ...
    inline void WorldAddV::DrawTileSymbols(Shared_Ptr<Tile> tile, Point coord) {
