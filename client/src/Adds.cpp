...
// Model
   ...
   void KeyboardMovementAdd::UpdateGameLogic() {
      ...
      try {
         ...
         auto time_to_update =
             SDL_GetTicks() > Hero::get()->ticks_last_move() + 400 / Hero::get()->movement_speed();

         if (time_to_update && (w_is_pressed || d_is_pressed || s_is_pressed || a_is_pressed)) {

            if (w_is_pressed)
               Hero::get()->MoveForward();

            if (d_is_pressed)
               Hero::get()->MoveRight();

            if (s_is_pressed)
               Hero::get()->MoveBackwards();

            if (a_is_pressed)
               Hero::get()->MoveLeft();

            Hero::get()->set_ticks_last_move(SDL_GetTicks());
            Hero::get()->set_destination({-1, -1});
         }
      }
   ...
   void MouseMovementAdd::UpdateGameLogic() {
      ...
      MouseInput::get()->left_btn()->AddFiredAction(
          [] { Hero::get()->set_destination(TileHoveringAdd::get()->hovered_tile()); }, 5);

      auto destination = Hero::get()->destination();
      ...
      if (time_to_update) {
         ...
         auto dx = destination.x + 0.5f - Hero::get()->pos().x;
         auto dy = destination.y + 0.5f - Hero::get()->pos().z;
         ...
         Hero::get()->set_facing_angle_deg(-base_angle_deg);

         auto angle_deg = base_angle_deg + Hero::get()->facing_angle_deg();
         Hero::get()->MoveAtAngle(angle_deg);
...
// MouseRotationModule
   ...
   void MouseRotationAdd::UpdateGameLogic() {
      ...
      if (is_rotating_) {
         ...
         Hero::get()->set_facing_angle_deg(Camera::get()->horizontal_angle_deg());
      }
   }
#endif
   ...
   void SkillPerformingAdd::UpdateGameLogic() {
      ...
      if (SDL_GetTicks() > ticks_last_skill_tick_ + 400 / skill_ticks_frequency_) {
         ...
         if (SDL_GetTicks() <
             ...
             Hero::get()->ticks_ulti_skill_start() != 0) {
