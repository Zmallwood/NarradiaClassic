...
namespace Narradia
{
   auto ModelPartConstr::TexNames(const aiScene *raw_model) const
   {
   ...
   auto ModelPartConstr::TexNameForMesh(const aiScene *raw_model, aiMesh *mesh) const
   {
   ...
   auto ModelPartConstr::Transformations(const aiScene *raw_model) const
   {
   ...
   auto ModelPartConstr::NodeTransformation(const aiScene *raw_model, std::string node_name) const
   {
   ...
   auto ModelPartConstr::Translate(Point3F *position, aiVectorKey position_keyframe) const
   {
   ...
   auto ModelPartConstr::NewModelPartKeyframe(
   ...
   auto ModelPartConstr::CreateModelPartFromMesh(
       ...
       -> std::shared_ptr<ModelPart>
   {
      ...
      auto no_keyframe_at_time_0_exists =
          KeyframeConstr::get()->PosKeyframe(raw_model, node_name, 0).mTime > 0;
      auto num_keyframes =
          KeyframeConstr::get()->NodePosKeyframes(raw_model, node_name).size();
      for (auto k = 0; k < num_keyframes; k++)
      {
         auto position_keyframe = KeyframeConstr::get()->PosKeyframe(raw_model, node_name, k);
         auto rotation_keyframe = KeyframeConstr::get()->RotKeyframe(raw_model, node_name, k);
         auto scaling_keyframe = KeyframeConstr::get()->ScalKeyframe(raw_model, node_name, k);
