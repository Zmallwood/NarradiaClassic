...
namespace Narradia {
    ...
    RendModelsV::RendModelsV()
        : timelines_(Make_Shared<Map<String_View, Map<float, Render_ID>>>()),
          model_ids_(Make_Shared<Map<String, Map<int, Map<float, const BodyData>>>>()) {
    ...
    Render_ID RendModelsV::NewBodyKeyframe(String_View model_name, float ms_time) {
        auto vao_id = renderer_base_->GenNewVAOId();

        if (timelines_->count(model_name) == 0)
            timelines_->insert({model_name, Map<float, Render_ID>()});
    ...
    void NewModel(String_View model_name) {
        ...
        for (auto &body : *model->model_parts()) {
            ...
            for (auto &keyframe : timelines) {
                ...
                auto body_keyframe_id =
                    RendModelsV::get()->NewBodyKeyframe(model_name, keyframe_time);
    ...
    void DrawModel(
        String_View model_name, float ms_time, Point3F position, float rotation, float scaling,
