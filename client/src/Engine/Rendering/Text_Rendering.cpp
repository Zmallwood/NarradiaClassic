...
namespace Narradia {
...
#if 1
    DrwTextV::DrwTextV()
        : unique_name_ids_(Make_Shared<std::map<Render_ID, std::string>>()) {
        TTF_Init();

        auto font_path =
            String(SDL_GetBasePath()) + kRelFontsPath + "PartyConfettiRegular-eZOn3.ttf";

        fonts_.insert({FontSizes::_20, Make_Shared<Font>(font_path.c_str(), 20)});
        fonts_.insert({FontSizes::_40, Make_Shared<Font>(font_path.c_str(), 40)});
    }
    ...
    // Font
#if 1
    Font::Font(String_View font_file_name, int font_size) {
        SDL_font_ =
            Shared_Ptr<TTF_Font>(TTF_OpenFont(font_file_name.data(), font_size), SDLDeleter());
...
// Free functions
    ...
    Render_ID NewString() {
        ...
        auto id = id_counter++;
        auto unique_name = "RenderedImage" + To_String(id);
    ...
    void DrawString(
        Render_ID rid, String_View text, PointF position, Color color, bool center_align,
        FontSizes font_size) {
        String unique_name_id;
        SizeF size;
        DrwTextV::get()->RenderText(
        ...
        int text_h;
        TTF_SizeText(
            DrwTextV::get()->fonts().at(font_size)->SDL_font().get(), text.data(), &text_w,
            &text_h);
