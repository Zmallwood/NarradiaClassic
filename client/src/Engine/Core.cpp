...
#include "Pages.h"
#include "Rendering/Images_Rendering.h"
...
#if 1
    IPage::IPage()
        : scene_gui_(Make_Shared<SceneGui>()) {
    }
...
// Cursor
    ...
    String_View Cursor::GetCursorImageName() {
        String_View img_name;
...
// KbInput
    ...
    void KbInput::AppendTextInput(String_View to_append) {
    ...
    void Console::Print(String_View text, Color text_color) {
        if (!enabled_)
            return;

        auto printed_text =
            String(CurrTime().data()) + "." + To_String(SDL_GetTicks() % 1000) + ") " + text.data();
    ...
    RectF ConsoleCalc::HorizontalSplitterRect() {
        return {
            ...
            Console::get()->Bounds().y + Console::get()->Bounds().h -
                1.3f * Console::kTextLineHeight,
...
#if 1
    IPageV::IPageV()
        : scene_gui_view_(Make_Shared<SceneGuiV>()) {
    }
...
// CursorV
    ...
    GraphicsV::GraphicsV() {
        ...
        auto win = Shared_Ptr<SDL_Window>(
            SDL_CreateWindow(
                Engine::get()->title().data(), 0, 0, win_size.w, win_size.h, win_flags),
            SDLDeleter());
    ...
    void ConsoleV::Render() const {
        ...
        DrawImage(
            "CommandLineInputArrow", rid_cmd_line_input_arrow_,
            ConsoleCalc::get()->InputArrowRect());
