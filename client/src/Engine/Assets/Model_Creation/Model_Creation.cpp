...
namespace Narradia {
    ...
    auto ModelPartCreator::TexNames(const aiScene *raw_model) const {
        ...
        for (auto i = 0; i < num_materials; i++) {
            ...
            raw_model->mMaterials[i]->Get(
                AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), tex_name_cstr);
    ...
    auto ModelPartCreator::NewModelPartKeyframe(
        ...
        auto new_model_part_keyframe = Make_Shared<Keyframe>();
    ...
    auto ModelPartCreator::CreateModelPartFromMesh(
        const aiScene *raw_model, String node_name, aiMesh *mesh) const -> Shared_Ptr<ModelPart> {
    ...
    auto MeshObtainer::ModelMeshIds(const aiScene *raw_model) const
        ...
        for (auto i = 0; i < num_sub_nodes; i++) {
            ...
            node_name_to_meshes.insert({Make_Shared<String>(key), meshes});
        }
        return node_name_to_meshes;
    }

    auto MeshObtainer::NodeMeshes(const aiScene *raw_model, Vector<int> node_mesh_ids) const
        -> Vector<aiMesh *> {
    ...
    auto ModelCreator::CreateModel(const aiScene *raw_model) -> Shared_Ptr<Model> {

        auto anim_duration =
            raw_model->mNumAnimations > 0 ? raw_model->mAnimations[0]->mDuration : 0;
