...
namespace Narradia {
   ...
   auto ModelPartCreator::TexNameForMesh(const aiScene *_rawModel, aiMesh *_mesh) const {
      auto _texNames = TexNames(_rawModel);
      auto _material = _mesh->mMaterialIndex;
      auto _texName = _texNames.at(_material);
      return _texName;
   }
   auto ModelPartCreator::Transformations(const aiScene *_rawModel) const {
   ...
   auto ModelPartCreator::NodeTransformation(const aiScene *_rawModel, String _nodeName) const {
   ...
   auto ModelPartCreator::Translate(Point3F *_pos, aiVectorKey _posKeyframe) const {
   ...
   auto ModelPartCreator::NewModelPartKeyframe(
      ...
      aiVectorKey _scalKeyframe) const {
      // clang-format on
      auto _newModelPartKeyframe = MakeShared<Keyframe>();
   ...
   auto ModelPartCreator::CreateModelPartFromMesh(
      ...
      for (auto _k = 0; _k < _numKeyframes; _k++) {
         auto _posKeyframe = KeyframeCreator::get()->PosKeyframe(_rawModel, _nodeName, _k);
         auto _rotKeyframe = KeyframeCreator::get()->RotKeyframe(_rawModel, _nodeName, _k);
         auto _scalKeyframe = KeyframeCreator::get()->ScalKeyframe(_rawModel, _nodeName, _k);
         ...
         _newModelPart->GetTimeln()->keyframes.insert({_keyframeTime, _newModelPartKeyframe});
         if (_noKeyframeAtTime0Exists && _k == 0)
            _newModelPart->GetTimeln()->keyframes.insert({0.0f, _newModelPartKeyframe});
      }
