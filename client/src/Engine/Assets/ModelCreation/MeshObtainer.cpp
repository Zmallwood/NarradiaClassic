...
namespace Narradia {
   auto MeshObtainer::ModelMeshIds(const aiScene *_rawModel) const
      ...
      Map<SharedPtr<String>, Vector<int>> _nodeNameToMeshes;
   ...
   auto MeshObtainer::NodeMeshes(const aiScene *_rawModel, Vector<int> _nodeMeshIDs) const
       -> Vector<aiMesh *> {
      Vector<aiMesh *> _nodeMeshes;
