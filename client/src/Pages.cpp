...
namespace Narradia {
    ...
    MainMenuPg::MainMenuPg() {
        scene_gui()->AddGuiComponent(
            Make_Shared<GuiButton>("Play", RectF{0.05f, 0.4f, 0.2f, 0.1f}, [] {
                PageMngrC::get()->ChangeScene(PageNames::MapCreation);
            }));

        scene_gui()->AddGuiComponent(Make_Shared<GuiButton>(
...
#if 1
    MainPg::MainPg() {
        scene_gui()->AddGuiComponent(Make_Shared<ExperienceBar>());
        scene_gui()->AddGuiComponent(Make_Shared<StatusPanel>());
    ...
    void MainPg::UpdateGameLogicDerived() {
        try {
            ...
            Console::get()->Print(
                "Exception in MainPg::UpdateGameLogicDerived: " + String(e.what()));
        }
    }
#endif
#endif
    ...
    MainMenuPgV::MainMenuPgV() {
        ...
        scene_gui_view_->AddGuiComponentView(Make_Shared<GuiButtonV>());
        scene_gui_view_->AddGuiComponentView(Make_Shared<GuiButtonV>());
    ...
    MainPgV::MainPgV() {
        ...
        scene_gui_view_->AddGuiComponentView(Make_Shared<ExperienceBarV>());
        scene_gui_view_->AddGuiComponentView(Make_Shared<StatusPanelV>());
        scene_gui_view_->AddGuiComponentView(Make_Shared<GuiWindowWorldMapV>());
    }
