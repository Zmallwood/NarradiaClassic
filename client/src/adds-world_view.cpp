...
// View
   ...
   void WorldViewAddV::InitCurrWorldArea() {
      ...
      auto map_offset_x = curr_map_loc.x * world_area->Width() * tile_size;
      auto map_offset_y = curr_map_loc.y * world_area->Height() * tile_size;
      auto inc = 1;
         ...
         for (auto y = y_center - r - 1; y <= y_center + r + 1; y++) {
            for (auto x = x_center - r - 1; x <= x_center + r + 1; x++) {
               ...
               if (x < 0 || y < 0 || x >= map_area->Width() || y >= map_area->Height()) {
                  ...
                  // West
                  if (x < 0 && y >= 0 && y < map_area->Height()) {
                     if (map_area_w) {
                        auto tile = map_area_w->GetTile(map_area->Width() + x, y);
                        if (tile) {
                           auto rid = tile->rid();
                           if (!rid) {
                              rid = NewTile();
                              ...
                              auto tile_size = kTileSize;
                              auto map_offset_x =
                                  curr_map_location.x * map_area->Width() * tile_size;
                              auto map_offset_y =
                                  curr_map_location.y * map_area->Height() * tile_size;
                              int x_orig = x;
                              int y_orig = y;
                              int x;
                              int y;
                              x = map_area->Width() + x_orig;
                              y = y_orig;
                              auto map_offset_x_orig = map_offset_x;
                              auto map_offset_y_orig = map_offset_y;
                              map_offset_x -= map_area->Width() * kTileSize;
                              Vertex3F v0;
                              Vertex3F v1;
                              Vertex3F v2;
                              Vertex3F v3;
