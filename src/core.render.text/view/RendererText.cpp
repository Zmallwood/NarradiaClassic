...
namespace Narradia {
    ...
    RendererText::RendererText() {
        ...
        auto font_path = std::string(SDL_GetBasePath()) + kRelFontsPath +
                         "PartyConfettiRegular-eZOn3.ttf";
        fonts_.insert(
            {FontSizes::_20, std::make_shared<Font>(font_path.c_str(), 20)});
        fonts_.insert(
    ...
    void RendererText::DrawString(
        ...
        int text_h;
        TTF_SizeText(
            fonts_.at(font_size)->SDL_font().get(), text.data(), &text_w,
            &text_h);
        rect.y -= static_cast<float>(text_h / GetAspectRatio()) /
                  canvas_size.height / 2.0f;
        if (center_align)
            rect.x -= static_cast<float>(text_w) /
                      static_cast<float>(canvas_size.height) / 2.0f /
                      GetAspectRatio();
    ...
    void RendererText::RenderText(
        ...
        auto text_outline_surface = TTF_RenderText_Blended(
            fonts_.at(font_size)->outline_SDL_font().get(), text.data(),
            outline_sdl_color);
        if (!text_outline_surface)
            return;
        auto text_surface =
            TTF_RenderText_Blended(font, text.data(), sdl_color);
        ...
        auto image = SDL_CreateRGBSurface(
            SDL_SWSURFACE, w, h, 32, 0x000000FF, 0x0000FF00, 0x00FF0000,
            0xFF000000);
        auto area = SDL_Rect{
            Font::kFontOutlineWidth, Font::kFontOutlineWidth,
            text_surface ? text_surface->w : 0,
        ...
        auto canvas_size = GetCanvasSize();
        auto width = text_surface ? static_cast<float>(text_surface->w) /
                                        canvas_size.width
                                  : 0;
        auto height = text_surface ? static_cast<float>(text_surface->h) /
                                         canvas_size.height
                                   : 0;
