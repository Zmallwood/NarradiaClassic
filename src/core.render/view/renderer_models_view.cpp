...
namespace Narradia {
  ...
  void RendererModelsView::NewBodyKeyframeGeometry(
    ...
    auto position_buffer_id = renderer_base_->GenerateNewBufferId(BufferTypes::Positions3D, vao_id);
    ...
    SetData(normal_buffer_id, num_vertices, normals.data(), BufferTypes::Normals, kLocationNormal);
    UseVaoEnd();
  }
  ...
  void RendererModelsView::DrawModel(
    ...
    if (p_model->anim_duration() == 0)
      ...
      ms_time_used = static_cast<int>(ms_time * global_animation_speed_) % p_model->anim_duration();
  ...
  void RendererModelsView::StartBatchDrawing() {
    ...
    glUniformMatrix4fv(
        location_projection_, 1, GL_FALSE, value_ptr(CameraGL::Get()->perspective_matrix()));
    glUniformMatrix4fv(location_view_, 1, GL_FALSE, glm::value_ptr(CameraGL::Get()->view_matrix()));
  }
