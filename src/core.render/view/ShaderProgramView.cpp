...
namespace Narradia {
    ...
    bool ShaderProgramView::Create(
        ...
        auto vertex_shader_res =
            DoCompileShader(vert_shader_src, &vertex_shader, GL_VERTEX_SHADER);
        auto success = true;

        if (vertex_shader_res != GL_TRUE) {
            SDL_ShowSimpleMessageBox(
                SDL_MESSAGEBOX_ERROR, "Shader error",
                "Unable to compile vertex shader.", nullptr);
        ...
        if (success) {
            ...
            auto fragment_shader_res = DoCompileShader(
                frag_shader_src, &fragment_shader, GL_FRAGMENT_SHADER);

            if (fragment_shader_res != GL_TRUE) {
                SDL_ShowSimpleMessageBox(
                    SDL_MESSAGEBOX_ERROR, "Shader error",
                    "Unable to compile fragment shader.", nullptr);
                printf(
        ...
        if (success) {
            ...
            glGetProgramiv(
                shader_program_->program_id(), GL_LINK_STATUS, &program_success);

            if (program_success != GL_TRUE) {
                SDL_ShowSimpleMessageBox(
                    SDL_MESSAGEBOX_ERROR, "Shader error",
                    "Error linking shader program.", nullptr);
                printf(
