...
#include "core.render.shaders/model/TilesFragment.glsl"
    ...
    RenderId RendererTilesView::NewImagePolygon(int num_vertices) {
        ...
        auto index_buffer_id =
            renderer_base_->GenerateNewBufferId(BufferTypes::Indices, vertex_array_id);
        auto position_buffer_id =
            renderer_base_->GenerateNewBufferId(BufferTypes::Positions3D, vertex_array_id);
        auto color_buffer_id =
            renderer_base_->GenerateNewBufferId(BufferTypes::Colors, vertex_array_id);
        auto uv_buffer_id = renderer_base_->GenerateNewBufferId(BufferTypes::Uvs, vertex_array_id);
        auto normal_buffer_id =
            renderer_base_->GenerateNewBufferId(BufferTypes::Normals, vertex_array_id);
        glUseProgram(GetShaderProgramView()->shader_program()->program_id());
        SetIndicesData(index_buffer_id, num_vertices, nullptr);
        SetData(position_buffer_id, num_vertices, nullptr, BufferTypes::Positions3D);
    ...
    void RendererTilesView::DrawTile(
        std::string_view image_name, RenderId vao_id, bool depth_test_off) const {
    ...
    void RendererTilesView::SetGeometryImagePolygon(
        ...
        auto position_buffer_id = renderer_base_->GetBufferId(BufferTypes::Positions3D, vao_id);
        auto color_buffer_id = renderer_base_->GetBufferId(BufferTypes::Colors, vao_id);
        auto uv_buffer_id = renderer_base_->GetBufferId(BufferTypes::Uvs, vao_id);
        auto normal_buffer_id = renderer_base_->GetBufferId(BufferTypes::Normals, vao_id);
        glBindVertexArray(vao_id);
        UpdateIndicesData(index_buffer_id, indices);
        UpdateData(position_buffer_id, positions, BufferTypes::Positions3D, kLocationPosition);
        ...
        UpdateData(normal_buffer_id, normals, BufferTypes::Normals, kLocationNormal);
        glBindVertexArray(0);
        if (!is_batch_drawing_)
            glUseProgram(0);
    }
    ...
    void RendererTilesView::DrawImagePolygon(
        std::string_view image_name, RenderId vao_id, int vertex_count, bool depth_test_off) const {
        if (depth_test_off)
