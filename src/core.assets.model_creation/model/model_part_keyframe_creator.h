...
namespace Narradia {
  class ModelPartKeyframeCreator : public Singleton<ModelPartKeyframeCreator> {
   public:
    ...
    auto GetNodePositionKeyframes(const aiScene *, std::string) const -> std::vector<aiVectorKey>;
    auto GetNodeRotationKeyframes(const aiScene *, std::string) const -> std::vector<aiQuatKey>;
    auto GetNodeScalingKeyframes(const aiScene *, std::string) const -> std::vector<aiVectorKey>;
