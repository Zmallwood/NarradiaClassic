...
namespace Narradia {
    void MouseRotationModule::UpdateGameLogic() {
        MouseInput::Get()->right_button()->AddFiredAction([&] {
            ...
            cam_horizontal_angle_deg_rotation_start_ =
                Camera::Get()->horizontal_angle_deg();
            cam_vertical_angle_deg_rotation_start_ =
                Camera::Get()->vertical_angle_deg();
        });

        MouseInput::Get()->right_button()->AddReleasedAction(
            [&] { is_rotating_ = false; });
        ...
        if (is_rotating_) {
            ...
            Player::Get()->set_facing_angle_deg(
                Camera::Get()->horizontal_angle_deg());
        }
    }
}
